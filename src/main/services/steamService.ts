...
export class SteamService {
  ...
  public setCustomSteamPath(customPath: string): { success: boolean; message: string } {
    if (!customPath || !fs.existsSync(customPath)) {
      return { success: false, message: `路径不存在: ${customPath}` };
    }
    if (!fs.existsSync(path.join(customPath, 'steam.exe'))) {
      return { success: false, message: '所选目录下未找到 steam.exe，请选择 Steam 安装根目录' };
    }
    this.customSteamPath = customPath;
    return { success: true, message: `Steam 路径已设置: ${customPath}` };
  }
  ...
  public async detectSteamPath(): Promise<string | null> {
    ...
    try {
      ...
      // 多个 steam.exe 运行时 stdout 为多行路径，逐行验证
      const runningLines = stdout.trim().split(/\r?\n/).map((l) => l.trim()).filter(Boolean);
      for (const runningExe of runningLines) {
        if (fs.existsSync(runningExe)) {
          const dir = path.dirname(runningExe);
          if (fs.existsSync(path.join(dir, 'steam.exe'))) {
            return dir;
          }
        }
      }
  ...
  public async checkEnvironmentHealth(): Promise<EnvironmentDiagnosticResult> {
    ...
    // 3. 配置文件 opensteamtool.toml 与规则目录 config/lua/（纯只读检测，不做任何写入）
    if (steamPath) {
      ...
      try {
        ...
        if (luaFiles.length === 0 && scriptFiles.length === 0) {
          items.push({
            name: '规则引擎目录 config/lua/',
            category: 'scripts',
            status: 'warning',
            message: '尚未部署任何入库规则',
            detail: '请先在库页面对游戏执行一键入库，或点击「一键修复」初始化规则环境'
          });
        } else {
          items.push({
            name: '规则引擎目录 config/lua/',
