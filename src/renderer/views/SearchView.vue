  ...
  <div class="h-full flex flex-col p-6 overflow-hidden">
    ...
    <div class="flex items-center gap-3.5 mb-5">
      <div class="relative flex-1">
        ...
        <button
          ...
          class="absolute right-3.5 top-3.5 text-slate-400 hover:text-slate-200 p-0.5 rounded-full hover:bg-slate-800 transition"
        >
          <X class="w-4 h-4" />
        </button>
      </div>
    ...
    <div class="flex-1 overflow-y-auto pr-1">
      <div v-if="loading && games.length === 0" class="flex flex-col items-center justify-center h-64 text-slate-400">
        ...
        <p class="text-xs">正在搜索 Steam 官方数据库与 18 万+ 云端端点...</p>
      </div>
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 pb-8">
        ...
        >
          ...
          <div class="relative w-full aspect-[460/215] bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 overflow-hidden flex items-center justify-center">
            ...
            <div class="absolute top-2.5 right-2.5 bg-slate-950/85 backdrop-blur-md px-2 py-0.5 rounded-lg text-[10px] font-mono theme-text-accent border border-white/10 font-bold">
              ID: {{ game.appId }}
            </div>
            <div
              v-if="unlockedAppIds.includes(game.appId)"
              class="absolute top-2.5 left-2.5 bg-emerald-600/90 backdrop-blur-md px-2.5 py-0.5 rounded-lg text-[10px] font-bold text-white shadow flex items-center gap-1"
            >
          ...
          <div class="p-4 flex-1 flex flex-col justify-between">
            <div>
              <h3 class="font-bold text-slate-100 text-xs line-clamp-1 group-hover:text-sky-300 transition duration-150">
                {{ game.nameZh || game.name }}
              </h3>
              <p class="text-[11px] text-slate-400 line-clamp-1 mb-1.5 font-mono">
                {{ game.name }}
              </p>
            ...
            <div class="pt-3 border-t border-slate-800/80 flex items-center justify-between gap-2">
              <button
                ...
                class="flex-1 py-2 theme-btn-primary disabled:opacity-50 text-xs font-bold rounded-xl shadow transition flex items-center justify-center gap-1.5"
              >
                <RotateCw v-if="unlockingId === game.appId" class="w-3.5 h-3.5 animate-spin" />
              ...
              <template v-else>
                <a
                  ...
                  class="flex-1 py-2 bg-gradient-to-r from-sky-600 to-blue-600 hover:from-sky-500 hover:to-blue-500 text-white text-xs font-bold rounded-xl shadow transition flex items-center justify-center gap-1.5"
                >
