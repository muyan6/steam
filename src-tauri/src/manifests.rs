...
/// 预取 GID 数量上限。
///
/// 预取仅用于主程序首次点击下载时的「秒开预热」，绝不应该试图在入库时把全部
/// 语言包和次要分包全部爬下来（如博德之门3有 53 个分包，爬完要等数分钟）。
/// 用户下载时 Steam 只拉取主程序与当前语言分包（1~2 个），其余全由 manifest.lua
/// 现场按需取码，因此这里仅预取前 3 个核心分包即可达到 100% 的预热价值，
/// 同时彻底杜绝超多分包游戏的入库卡顿。
const PREFETCH_MAX_GIDS: usize = 3;
/// 单批并发度：与上限保持一致（3），所有核心分包在单个批次内并发发出，零分批排队。
const PREFETCH_CONCURRENCY: usize = 3;
/// 快速探针超时（秒）：入库是用户界面交互路径，绝不能阻塞 12 秒。
/// 热缓存（中继/古韵 HIT）通常在 0.2~0.8 秒内返回；2 秒未返回说明对端挂起或不可用，
/// 必须快速放弃并放行入库，绝不阻塞用户界面。
const PREFETCH_TIMEOUT_SECS: u64 = 2;
