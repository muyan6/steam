...
/// 预取 GID 数量上限。
///
/// 原为 16，理由是「真实游戏分包通常 4~11 个」—— 实测这个假设是错的：
/// 龙之信条 2（AppID 2054970）带 GID 的 depot 有 **35 个**，16 只覆盖不到一半。
/// 后果直接体现在用户侧：剩下 19 个分包必须现场取码，每个要串行走完三个源
/// （实测 1.05 秒），累计 ~20 秒的等待 —— 这就是「点了下载但很久不动」的来源。
///
/// 提到 64：覆盖已知最大的游戏（35）仍留一倍余量，同时把最坏情况限制在
/// 64 / 并发3 x 单请求12秒 ≈ 256 秒（仅在三个源全部超时的病态情况下才会发生；
/// 正常路径单请求 0.2~1 秒，总耗时 5~20 秒）。
const PREFETCH_MAX_GIDS: usize = 64;
